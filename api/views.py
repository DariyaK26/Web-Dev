...
def restaurant_tables(request, restaurant_id):
    ...
    bookings = Booking.objects.filter(restaurant_id=restaurant_id)

    data = []
    for table in tables:
        is_booked = bookings.filter(table=table).exists()
        data.append({
            'id': table.id,
            'size': table.size,
...
def table_bookings(request, table_id):
    bookings = Booking.objects.filter(table_id=table_id)
    data = [
        {
            'date': b.date,
            'time': b.time,
...
def available_tables(request, restaurant_id):
    ...
    # Свободные = все, кроме тех, что в брони
    available = all_tables.exclude(id__in=booked_tables)
...
def restaurant_reviews(request, restaurant_id):
    reviews = Review.objects.filter(restaurant_id=restaurant_id).select_related('user')
    data = [
        {
            'rating': r.rating,
            'comment': r.comment,
